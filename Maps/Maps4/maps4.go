...
func getNameCounts(names []string) map[rune]map[string]int {
	...
	for _, name := range names {
		...
		currentCharacter := rune(name[0])

		//fmt.Println("Current character: ", currentCharacter)

		// If the current character not exists in the returned map
		if _, ok := nameCounts[currentCharacter]; !ok {

			letterNames := make(map[string]int)
			nameCounts[currentCharacter] = letterNames

		}

		nameCounts[currentCharacter][name]++
